    ...
    def document_filter(self, elastic_document):
        ...
        for field in self.filter_fields:
            if field in self.request.query_params.keys():
                elastic_document = self.filter_document_queryset(
                    elastic_document.query('match', **{field: self.request.query_params[field]})
                )
                was_filtered = True
        if not was_filtered:
    ...
    def list(self, request, *args, **kwargs):
        ...
        if self.request.user.is_authenticated and getattr(self, 'add_metadata', None):
            queryset = self.child_document.search().query(
                Q(Q('nested', path='created_by', query=Q('match', created_by__id=self.request.user.id)) &
                  Q('match', status=Status.PENDING)))

            if self.request.query_params.get('search', None):
                queryset = queryset.filter(
                    Q("match_phrase_prefix", title=self.request.query_params.get('search', None))
                )
            queryset = self.filter_metaclass_queryset(queryset)
            response = response.data.copy()
            response.update({"metadata": {
                "pending_approval": self.child_serializer(queryset[0:5000].to_queryset(), many=True).data
            }})
...
class DotsModelMixin(models.Model):
    created_by = models.ForeignKey(User, null=True, on_delete=models.SET_NULL, verbose_name=_('Creator'),
                                   related_name='%(class)s_create_user')
    updated_by = models.ForeignKey(User, null=True, on_delete=models.SET_NULL, verbose_name=_('Editor'),
